...
public class TestDefaultInvoiceItemFormatter extends InvoiceTestSuiteNoDB {
    ...
    private void checkOutput(final InvoiceItem invoiceItem, final String template, final String expected, final Locale locale) {
        ...
        data.put("invoiceItem", new DefaultInvoiceItemFormatter(config.getDefaultLocale(), config.getCatalogBundlePath(), invoiceItem,  DateTimeFormat.mediumDate().withLocale(locale), locale, internalCallContext, resourceBundleFactory)); //TODO_custom_invoice - check if correct
